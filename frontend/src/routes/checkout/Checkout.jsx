...
/**
 * Thành phần thanh toán để xử lý việc thanh toán và thu thập thông tin người dùng.
 *
 * @returns {JSX.Element} Thành phần Checkout được hiển thị.
 */
...
const Checkout = () => {
 ...
 // Trạng thái biểu mẫu để thu thập thông tin thanh toán và địa chỉ của người dùng
  ...
  // Định dạng ngày giờ nhận phòng và trả phòng
  ...
  /**
   * Xử lý thay đổi dữ liệu nhập vào và kiểm tra hợp lệ.
   * @param {React.ChangeEvent<HTMLInputElement>} e Sự kiện thay đổi đầu vào.
   */
 ...
 /**
   * Xử lý gửi biểu mẫu và kiểm tra hợp lệ.
   * @param {React.FormEvent<HTMLFormElement>} e Sự kiện gửi biểu mẫu.
   * @returns {void}
   * @todo Triển khai logic gửi biểu mẫu.
   * @todo Triển khai logic kiểm tra hợp lệ biểu mẫu.
   * @todo Triển khai xử lý lỗi khi gửi biểu mẫu.
   * @todo Triển khai xử lý thành công khi gửi biểu mẫu.
   * @todo Triển khai trạng thái tải khi gửi biểu mẫu.
   * @todo Triển khai trạng thái lỗi khi gửi biểu mẫu.
   */
  const handleSubmit = async (e) => {
    ...
    if (!isValid) {
      return; // Dừng gửi biểu mẫu nếu có lỗi
    }
...
const InputField = ({
  ...
  <div className="mb-4">
    ...
    />
    {error && (
      <p className="text-red-500 text-xs my-1">Vui lòng kiểm tra trường này.</p>
    )}
  </div>
);
